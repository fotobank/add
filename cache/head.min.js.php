...
header( 'Expires: Fri, 09 Nov 2018 09:37:13 GMT' );
header( 'Last-Modified: Mon, 08 Oct 2018 21:54:58 GMT' );
...
/* Filename: bootstrap-modalmanager.js */
...
if(!window.dhtmlx)
window.dhtmlx={};(function(){var _dhx_msg_cfg=null;function callback(config,result){var usercall=config.callback;modality(false);config.box.parentNode.removeChild(config.box);_dhx_msg_cfg=box=config.box=null;if(usercall)
usercall(result);}
function modal_key(e){if(_dhx_msg_cfg){var code=e.which||event.keyCode;if(dhtmlx.message.keyboard){if(code==13||code==32)
callback(_dhx_msg_cfg,true);if(code==27)
callback(_dhx_msg_cfg,false);}
if(e.preventDefault)
e.preventDefault();return!(e.cancelBubble=true);}};if(document.attachEvent)
document.attachEvent("onkeydown",modal_key);else
document.addEventListener("keydown",modal_key,false);function modality(mode){if(!modality.cover){modality.cover=document.createElement("DIV");modality.cover.className="dhx_modal_cover";document.body.appendChild(modality.cover);}
var height=document.body.scrollHeight;modality.cover.style.display=mode?"inline-block":"none";}
function button(text,callback){return"<div class='dhtmlx_popup_button' "+(callback?"result='true' ":"")+"><div>"+text+"</div></div>";}
...
function _createBox(config,ok,cancel){var box=document.createElement("DIV");box.className=" dhtmlx_modal_box dhtmlx-"+config.type;var inner='';if(config.title)
inner+='<div class="dhtmlx_popup_title">'+config.title+'</div>';inner+='<div class="dhtmlx_popup_text"><span>'+config.text+'</span></div><div  class="dhtmlx_popup_controls">';if(ok)
inner+=button(config.ok||"OK",true);if(cancel)
inner+=button(config.cancel||"Cancel",false);inner+='</div>';box.innerHTML=inner;box.onclick=function(e){e=e||event;var source=e.target||e.srcElement;if(!source.className)source=source.parentNode;if(source.className=="dhtmlx_popup_button")
callback(config,source.getAttribute("result")=="true");};config.box=box;_dhx_msg_cfg=config;modality(true,box);document.body.appendChild(box);var x=Math.abs(Math.floor(((window.innerWidth||document.documentElement.offsetWidth)-box.offsetWidth)/2));var y=Math.abs(Math.floor(((window.innerHeight||document.documentElement.offsetHeight)-box.offsetHeight)/2));box.style.top=y+'px';box.style.left=x+'px';box.focus();}
function _popupButtonClick(config,param){return function(){t.hide(config.id);if(typeof config.callback=="function")
config.callback(param);};}
function alertPopup(config){var box=_createBox(config,true,false);}
function confirmPopup(config){var box=_createBox(config,true,true);}
...
/* Filename: humane.js */
;(function(win,doc){var on,off,isArray,eventing=false,animationInProgress=false,humaneEl=null,timeout=null,useFilter=/msie [678]/i.test(navigator.userAgent),vendors={Webkit:'webkit',Moz:'',O:'o',ms:'MS'},eventPrefix="",isSetup=false,queue=[],after=null;if('addEventListener'in win){on=function(obj,type,fn){obj.addEventListener(type,fn,false)};off=function(obj,type,fn){obj.removeEventListener(type,fn,false)};}
else{on=function(obj,type,fn){obj.attachEvent('on'+type,fn)};off=function(obj,type,fn){obj.detachEvent('on'+type,fn)};}
isArray=Array.isArray||function(obj){return Object.prototype.toString.call(obj)==='[object Array]'};function normalizeEvent(name){return eventPrefix?eventPrefix+name:name.toLowerCase();}
on(win,'load',function(){var transitionSupported=(function(style){var prefixes=['MozT','WebkitT','OT','msT','KhtmlT','t'];for(var i=0,prefix;prefix=prefixes[i];i++){if(prefix+'ransition'in style)return true;}
return false;}(doc.body.style));if(!transitionSupported)animate=jsAnimateOpacity;setup();run();});function setup(){humaneEl=doc.createElement('div');humaneEl.id='humane';humaneEl.className='humane';doc.body.appendChild(humaneEl);for(vendor in vendors){if(humaneEl.style[vendor+'TransitionProperty']!==undefined)
eventPrefix=vendors[vendor];}
isSetup=true;}
function remove(){off(doc.body,'mousemove',remove);off(doc.body,'click',remove);off(doc.body,'keypress',remove);off(doc.body,'touchstart',remove);eventing=false;if(humane.clickToClose){off(humaneEl,'click',remove);off(humaneEl,'touchstart',remove);}
if(animationInProgress)animate(0);}
function run(){if(animationInProgress&&!win.humane.forceNew)return;if(!queue.length){remove();return;}
after=null;animationInProgress=true;if(timeout){clearTimeout(timeout);timeout=null;}
timeout=setTimeout(function(){if(!eventing){on(doc.body,'mousemove',remove);on(doc.body,'click',remove);on(doc.body,'keypress',remove);on(doc.body,'touchstart',remove);eventing=true;if(!win.humane.waitForMove)remove();}},win.humane.timeout);if(humane.clickToClose){on(humaneEl,'click',remove);on(humaneEl,'touchstart',remove);}
var next=queue.shift(),type=next[0],content=next[1],callback=next[2];after=callback;if(isArray(content))content='<ul><li>'+content.join('<li>')+'</ul>';humaneEl.innerHTML=content;animate(type,1);}
function animate(type,level){if(level===1){humaneEl.className="humane humane-"+type+" humane-animate";}
else{humaneEl.className=humaneEl.className.replace(" humane-animate","");if(after!=null)
on(humaneEl,normalizeEvent('TransitionEnd'),after);end();}}
function end(){setTimeout(function(){animationInProgress=false;run();},500);}
var setOpacity=(function(){if(useFilter){return function(opacity){humaneEl.filters.item('DXImageTransform.Microsoft.Alpha').Opacity=opacity*100;}}
else{return function(opacity){humaneEl.style.opacity=String(opacity);}}}());function jsAnimateOpacity(type,level){var interval;var opacity;if(level===1){opacity=0;humaneEl.className="humane humane-js-animate humane-"+type;if(humaneEl.filters)humaneEl.filters.item('DXImageTransform.Microsoft.Alpha').Opacity=0;if(win.humane.forceNew){opacity=useFilter?humaneEl.filters.item('DXImageTransform.Microsoft.Alpha').Opacity/100|0:humaneEl.style.opacity|0;}
interval=setInterval(function(){if(opacity<1){opacity+=0.1;if(opacity>1)opacity=1;setOpacity(opacity);}
else{clearInterval(interval);}},100/20);}
else{opacity=1;interval=setInterval(function(){if(opacity>0){opacity-=0.1;if(opacity<0)opacity=0;setOpacity(opacity);}
else{humaneEl.className=humaneEl.className.replace(" humane-js-animate","");clearInterval(interval);if(after!=null)
after();end();}},100/20);}}
function notifier(type){return function(message,callback){queue.push([type,message,callback]);if(isSetup)run();}}
win.humane=notifier('log');win.humane.log=notifier('log');win.humane.error=notifier('error');win.humane.info=notifier('info');win.humane.success=notifier('success');win.humane.timeout=2500;win.humane.waitForMove=false;win.humane.forceNew=false;win.humane.clickToClose=false;}(window,document));

/* Filename: main.js */

(function($){$.fn.outDebug=function(msg,title,group){return this.each(function(){window.hackerConsole=window.hackerConsole||window.Debug_HackerConsole_Js&&new window.Debug_HackerConsole_Js();if(window.hackerConsole)setTimeout(function(){with(window.hackerConsole){out(msg,title,group);}},200);});};$(function(){$(".vhod , input, textarea, label, .lazy, .img3").tooltip();});$("a[rel=popover]").popover({offset:10}).click(function(e){e.preventDefault()});function hPocta(){var data=$('#nPocta').val().split('|');if(data[0]!='выбрать')
{var val="<p style='font-size:12px;'>Обращаем Ваше внимание, что при выборе наложенного платежа заказчик оплачивает пересылку фотографий в одну сторону и денег в другую. С ценами на почтовые услуги Вы можете ознакомиться кликнув по ссылке.</p><a href= '"+data[1]+"' class='ttext_blue' style='font-size:10px; float: right;' target='_blank'>Расчет стоимости пересылки в почтовой службе  '"+data[0]+"' </a>";$(' #httpPocta ').empty().html(val);}}
$.fn.autoClear=function(){$(this).each(function(){$(this).data("autoclear",$(this).attr("value"));});$(this).bind('focus',function(){if($(this).attr("value")==$(this).data("autoclear")){$(this).attr("value","").addClass('autoclear-normalcolor');}}).bind('blur',function(){if($(this).attr("value")==" "){$(this).attr("value",$(this).data("autoclear")).removeClass('autoclear-normalcolor');}});return $(this);};$(function(){$('.autoclear, .inp_f_reg').autoClear();});$(function(){$('[data-toggle="modal"]').click(function(e){e.preventDefault();var url=$(this).attr('href');if(url.indexOf("#")==0){$(url).modal('open');}else{$.get(url,function(data){$('<div id="vosst" class="modal hide fade in animated fadeInDown" style="position: absolute; top: 40%; left: 50%; z-index: 1; " data-keyboard="false" data-width="460" data-backdrop="static" tabindex="-1" aria-hidden="false">'+data+'</div>').modal();}).success(function(){$(' .autoclear ').autoClear().tooltip();});}});});$(function(){$('[data-toggle="order"]').click(function(e){e.preventDefault();var url=$(this).attr('href');if(url.indexOf("#")==0){$(url).modal('open');}else{$.get(url,function(data){$('<div id="vosst" class="modal hide fade in animated fadeInDown" style="position: absolute; top: 40%; left: 50%; z-index: 1; " data-keyboard="false" data-width="950" data-backdrop="static" tabindex="-1" aria-hidden="false">'+data+'</div>').modal();})}});});})(jQuery);function scrambleVideo(){var pa,pb,pc,pd,pe,pf;pa='<a href='+'"mai';pb='video';pc='">';pa+='lto:';pb+='@';pe='</a>';pf='Заказ видеосъемки';pb+='aleks.od.ua';pd=pf;document.write(pa+pb+pc+pd+pe)}
function scrambleFoto(){var pa,pb,pc,pd,pe,pf;pa='<a href='+'"mai';pb='foto';pc='">';pa+='lto:';pb+='@';pe='</a>';pf='Заказ фотосесии';pb+='aleks.od.ua';pd=pf;document.write(pa+pb+pc+pd+pe)}
function smile(str){obj=document.Sad_Raven_Guestbook.mess;obj.focus();obj.value=obj.value+str;}
function openBrWindow(theURL,winName,features){window.open(theURL,winName,features);}
function inserttags(stT,enT){obj=document.Sad_Raven_Guestbook.mess;obj2=document.Sad_Raven_Guestbook;if((document.selection)){obj.focus();obj2.document.selection.createRange().text=stT+obj2.document.selection.createRange().text+enT;}
else{obj.focus();obj.value+=stT+enT;}}

/* Filename: ajax.js */

function ajaxPostQ(url,idName,data){$(idName).empty();$(document).ready(function(){$(idName).append('<div id="ajaxBusy"><p><img src="/img/loading2.gif"></p></div>');$('#ajaxBusy').css({display:"none",position:"relative",left:"30px",width:"auto"});});$(document).ajaxStart(function(){$('#ajaxBusy').show();}).ajaxStop(function(){$('#ajaxBusy').hide();});$.ajax({type:"POST",header:('Content-Type: application/json; charset=utf-8;'),url:url,data:data,error:function(XHR){$(this).outDebug(" Ошибка: "+XHR.status+"  "+XHR.statusText,url,"ajaxPostQ");},statusCode:{404:function(){$(this).outDebug("Страница не найдена",url,"ajaxPostQ");}},success:function(html){$(idName).empty().append(html);}});}
function onJS(url){$.ajax({type:"POST",header:('Content-Type: application/json; charset=utf-8;'),url:url,data:{js:1},error:function(XHR){$(this).outDebug(" Ошибка: "+XHR.status+"  "+XHR.statusText,url,"onJS");},statusCode:{404:function(){$(this).outDebug("Страница не найдена",url,"ajaxPostQ");}},success:function(html){}});}
function reload(cfg,SID){$(" #cryptogram ").attr("src","/classes/dsp/cryptographp.php?cfg="+cfg+"&"+SID+"&"+Math.round(Math.random(0)*1000)+1);}
function returnCaptcha(){reload('kontakti.cfg.php','.<?=SID?>.')}
function goKorzDel(idName,srt){$('#ramka'+idName).empty().html("<div style='margin:25px 0 0 35px;'><img style='width: 140px; float: left; margin-left: 5px;' src= '/img/not_foto.png'></div>");$('#iTogo').empty().load('/inc/ajaxZakazDel.php',{goZakazDel:idName,str:srt},function(data){$(this).outDebug(data,"/inc/ajaxZakazDel.php","goKorzDel");if(!data)
...
function goPodpiska(album){$('#podpiska').empty().load('/inc/ajaxPodpiska.php',{album:album},function(data){$(this).outDebug(data,"/inc/ajaxPodpiska.php","goPodpiska");if(data)
{$('#podpiska').empty().html(data);}});}
function ajaxAdd(data){$.ajax({type:"POST",header:('Content-Type: application/json; charset=utf-8;'),url:'/inc/ajaxZakazDel.php',data:data,error:function(XHR){$(this).outDebug(" Ошибка: "+XHR.status+"  "+XHR.statusText,"/inc/ajaxZakazDel.php","ajaxAdd");},statusCode:{404:function(){$(this).outDebug("Страница не найдена","/inc/ajaxZakazDel.php","ajaxAdd");}},success:function(html){$(this).outDebug(html,"/inc/ajaxZakazDel.php","ajaxAdd");var ans=JSON.parse(html);if(ans.add==1){dhtmlx.message({text:"Фотография № "+ans.nm+"<br> добавленна в корзину",expire:9000,type:"addfoto"});}else{dhtmlx.message({text:"Фотография № "+ans.nm+"<br> удалена из корзины",expire:12000});}
if(ans.fDel==1){if(!ans.prKoll)
{$('#clearStr').empty().html("<div class='drop-shadow lifted' style='margin: 50px 0 0 480px;'><div style='font-size: 24px;'>Ваша корзина пуста!</div></div>");}
$('#ramka'+ans.id).empty().html("<div style='margin:25px 0 0 35px;'><img style='width: 140px; float: left; margin-left: 5px;' src= '/img/not_foto.png'></div>")}else{$('#fKoll'+ans.id).empty().append(ans.fKoll+' шт');$('#fSumm'+ans.id).empty().append(ans.fSumm+' грн');}
$('#iTogo').empty().append('ИТОГО: '+ans.sum+'  гривень  ('+ans.prKoll+' фото '+ans.format+' см)');}});}
function ajaxFormat(data){$.ajax({type:"POST",header:('Content-Type: application/json; charset=utf-8;'),url:'/inc/ajaxZakazDel.php',data:data,error:function(XHR){$(this).outDebug(" Ошибка: "+XHR.status+"  "+XHR.statusText,"/inc/ajaxZakazDel.php","ajaxFormat");},statusCode:{404:function(){$(this).outDebug("Страница не найдена","/inc/ajaxZakazDel.php","ajaxFormat");}},success:function(html){$(this).outDebug(html,"/inc/ajaxZakazDel.php","ajaxFormat");var ans=JSON.parse(html);dhtmlx.message({text:"Выбран формат: <br> "+ans.format+" см",expire:9000,type:"addfoto"});$.each(ans.prArr,function(i,n){$('#fCena'+i).empty().append(n+' грн');});$.each(ans.summArr,function(i,n){$('#fSumm'+i).empty().append(n+' грн');});$('#iTogo').empty().append('ИТОГО: '+ans.sum+'  гривень  ('+ans.prKoll+' фото '+ans.format+' см)');}});}
jQuery.extend({php:function(url,params){$.ajax({url:url,type:"POST",data:params,dataType:"json"})}});function getCaptca(){$.ajax({type:"GET",header:('Content-Type: application/json; charset=utf-8;'),url:"/inc/captcha/captcha.html",cache:false,error:function(xhr,status,errorThrown){alert(errorThrown+'\n'+status+'\n'+xhr.statusText);},success:function(data){$(this).outDebug(data,"/inc/captcha/captcha.html","getCaptca");$(" #loadCaptca ").empty().append(data);return false;}});}
...
{var xmlhttp=getXmlHttp();xmlhttp.open('post',url,true);xmlhttp.onreadystatechange=function(){var dataState=xmlhttp.readyState;if(dataState==4)
{idElement(idName).innerHTML=xmlhttp.responseText;}
else
{idElement(idName).innerHTML='<br><div style="text-align: center;">Запрос ...</div>';}};xmlhttp.setRequestHeader("Content-Type","application/x-www-form-urlencoded; charset=utf-8");xmlhttp.send(data);}
function ajaxGet(url,idName)
{var xmlhttp=getXmlHttp();xmlhttp.open("get",url+'?sl&rn='+Math.random(),true);xmlhttp.onreadystatechange=function(){var dataState=xmlhttp.readyState;if(dataState==4)
{idElement(idName).innerHTML=xmlhttp.responseText;}
else
{idElement(idName).innerHTML='<br><div style="text-align: center;">Запрос ...</div>';}};xmlhttp.setRequestHeader("Content-Type","application/x-www-form-urlencoded; charset=utf-8");xmlhttp.send();}
