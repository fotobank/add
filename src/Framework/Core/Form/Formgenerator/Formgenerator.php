...
use Framework\Core\Form\Formvalidator\Formvalidator;
       ...
       class Formgenerator {
              ...
                             * errorPosition <enum> указать положение окна с ошибкой [до | после | in_before | in_after]
                             * («before» и «after» помещает ящик вне элемента формы, параметры in_ * помещают его внутрь)
...
//                                          if (class_exists($this->getConfig('validatorClass'))) {
                            ...
                            if ($function !== NULL && method_exists($this->validator, $function)) {
                                   ...
                                   throw new \RuntimeException('Метод &quot;'.$function.'&quot; не найден.');
                            }
                     }
                     catch (\RuntimeException $e) {
                            $this->debug('Ошибка в '.$this->getConfig('name').' Form->validator: '.$e->getMessage());
                     }
              }
              /*
