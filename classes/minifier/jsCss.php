...
**    'gzip' => true
** );
** $minified = new minifier_jsCss( $vars );
**
...
class minifier_jsCss {
    ...
    public function __construct( $vars = array() )
    {
        ...
        {
            $this->messages[]['minifier_jsCss Log'] = 'Echo for output';
            $this->print = false;
        }
        else
        {
        ...
        {
            $this->messages[]['minifier_jsCss Log'] = 'Base64encoding enabled';
        ...
        {
            $this->messages[]['minifier_jsCss Log'] = 'Gzip enabled';
            $this->gzip = true;
        }
     ...
     * Matthias Mullie <minify@mullie.eu>'s function, "importFiles" from the JavaScript and CSS minifier_jsCss
    ...
    private function merge_images( $source_file, $contents )
    {
        ...
        if( preg_match_all( '/url\((["\']?)((?!["\']?data:).*?\.(gif|png|jpg|jpeg))\\1\)/i', $contents, $this->matches, PREG_SET_ORDER ) )
        {
            ...
            foreach( $this->matches as $this->graphic )
            {
                ...
                if( $this->remote_file( $this->graphic[2] ) )
                {
                    ...
                    if( curl_exec( $ch ) !== false )
                    {
                        ...
                        if( $this->filesize <= minifier_jsCss::IMAGE_MAX_SIZE * 1024 )
                        {
            ...
            $this->messages[]['minifier_jsCss Log: merge_images'] = count( $this->replace ) .' files base64_encoded into ' . $source_file;
    ...
    private function minify_contents( $src_file, $run_minification = true )
    {
        ...
        if( !$this->source )
        {
            $this->messages[]['minifier_jsCss ERROR'] = 'Unable to retrieve the contents of '. $src_file . '.  Skipping at '. __LINE__ .' in '. basename( __FILE__ );
    ...
    private function make_min( $src_file, $new_file, $do_minify = true )
    {
        ...
        else
        {
            ...
            $filenames = array_map( array( 'minifier_jsCss', 'strip_directory' ), $src_file );
        ...
        //Create the new file
		  $this->handle = fopen( $_SERVER['DOCUMENT_ROOT'] . '/' . $new_file, 'w' );
        
        //Log any error messages from the new file creation
        if( !$this->handle )
        {
            $this->messages[]['minifier_jsCss ERROR'] = 'Unable to open file:  '.$new_file;
            return false;
        }
        else
        {
            ...
            //Log to the console
            $this->messages[]['minifier_jsCss Log: New file'] = 'Successfully created '. $new_file;
    ...
    public function minify( $src_file, $file = '', $version = '' )
    {
        ...
        if( $this->remote_file( $_SERVER['DOCUMENT_ROOT'].$src_file ) && file_exists( $_SERVER['DOCUMENT_ROOT'].$file ) )
        {
            $this->output_file = $file;
        }
        //The local version doesn't exist, but we don't need to minify
        elseif( $this->remote_file( $_SERVER['DOCUMENT_ROOT'].$src_file ) && !file_exists(  $_SERVER['DOCUMENT_ROOT'].$file ) )
        {
            $this->output_file = $this->make_min( $_SERVER['DOCUMENT_ROOT'].$src_file, $file, false );
            
            //Add the filename to the output log
            $this->messages[]['minifier_jsCss Log: minify'] = 'Retrieving contents of '.$src_file .' to add to '.$file;
        }
        //The file already exists and doesn't need to be recreated
        elseif( ( file_exists( $_SERVER['DOCUMENT_ROOT'].$file ) && file_exists( $_SERVER['DOCUMENT_ROOT'].$src_file ) ) && ( filemtime( $_SERVER['DOCUMENT_ROOT'].$src_file ) < filemtime( $_SERVER['DOCUMENT_ROOT'].$file ) ) )
        {
        ...
        elseif( ( file_exists( $_SERVER['DOCUMENT_ROOT'].$file ) && file_exists( $_SERVER['DOCUMENT_ROOT'].$src_file ) ) && ( filemtime( $_SERVER['DOCUMENT_ROOT'].$src_file ) > filemtime( $_SERVER['DOCUMENT_ROOT'].$file ) ) )
        {
            ...
            $this->output_file = $this->make_min( $_SERVER['DOCUMENT_ROOT'].$src_file, $file );
            
            //Add to the console.log output
            $this->messages[]['minifier_jsCss Log: minify'] = 'Made new version of '.$src_file.' into '.$file;
        }
     ...
     * @param array $order to specify output order -> определить порядок вывода
  ...
  public function merge( $output_filename, $directory, $list_or_type = 'js', $exclude = array(), $order = array())
    {
        ...
        if( !empty( $order ) )
        {
            ...
            foreach( $order as $key => $this->specified['file'] )
            {

              //Check each file for modification greater than the output file if it exists
				  if( file_exists( $output_filename ) && ( $this->specified['file'] != $output_filename ) && ( !$this->remote_file( $this->specified['file'] ) )
					  && ( filemtime( $this->specified['file'] ) > filemtime( $output_filename ) ) )
                {
                    $this->messages[]['minifier_jsCss Log: New File Flagged'] = 'Flagged for update by '. $this->specified['file'];
                    $this->create_new = true;
                }
                
                //Add the specified files to the beginning of the use array passed to $this->make_min
                $this->compilation[] = $this->specified['file'];
        ...
        foreach( $this->directory as $this->file )
        {
            ...
            if( !in_array( $this->file, $exclude ) && ( $this->file != $output_filename ) )
            {
                ...
                {
                    $this->messages[]['minifier_jsCss Log: New File Flagged'] = 'Flagged for update by '. $this->file;
                    $this->create_new = true;
                }
        ...
        if( $this->create_new || !file_exists(  $_SERVER['DOCUMENT_ROOT'] . $output_filename ) )
        {
     ...
     * Usage (assuming minifier_jsCss is initiated as $minifier_jsCss):
     * <?php $minifier_jsCss->logs(); ?>
     *
     * @param none
     * @return string
     */
    public function logs()
    {
        ...
        if( $this->timer )
        {
            $this->messages[]['minifier_jsCss Log: timer'] = 'MagicMin processed and loaded in '. ( microtime( true ) - $this->mtime ) .' seconds';
        }
