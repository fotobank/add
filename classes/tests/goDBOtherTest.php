...
class goDBOtherTest extends goDBTestBase {
    ...
    public function testDecorator() {
        ...
        $r2 = go\DB\query('SELECT `id` FROM `godb` LIMIT 2', null, 'num');
        $r3 = go\DB\query('SELECT `id` FROM `godb` LIMIT 3', null, 'num');
        $r4 = go\DB\query('SELECT `id` FROM `godb` LIMIT 4', null, 'num');
        $this->assertEquals(2, $r2);
        $this->assertEquals(3, $r3);
        $this->assertEquals(4, $r4);

        go\DB\queryDecorated($decor->getCallback());
        $r2 = go\DB\query('SELECT `id` FROM `godb` LIMIT 2', null, 'num');
        $r3 = go\DB\query('SELECT `id` FROM `godb` LIMIT 3', null, 'num');
        $r4 = go\DB\query('SELECT `id` FROM `godb` LIMIT 4', null, 'num');
        $this->assertEquals(2, $r2);
