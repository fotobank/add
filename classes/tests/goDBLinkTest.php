...
class goDBLinkTest extends goDBTestBase {
    ...
    public function testDebug() {
        ...
        $this->assertEquals($expected, go\DB\query($pattern, $data, 'el'));
    ...
    public function testDecorator() {
        ...
        go\DB\queryDecorated($w1->getCallback());
        $link->queryDecorated($w2->getCallback());

        $pattern = 'SELECT `id` FROM `godb` WHERE `id`=?i';

        $this->assertEquals(4, go\DB\query($pattern, array(3), 'el'));
        $this->assertEquals(4, go\DB\query($pattern, array(4), 'el'));
        $this->assertEquals(false, go\DB\query($pattern, array(5), 'el'));
        $this->assertEquals(6, go\DB\query($pattern, array(6), 'el'));
