...
class goDBTransactionTest extends goDBTestBase {
    ...
    public function testMysqli() {
        ...
        $count1 = go\DB\query($patternS, null, 'el');

        $db->autocommit(false);

        go\DB\query($patternI);
        $count2 = go\DB\query($patternS, null, 'el');
        $this->assertEquals($count1 + 1, $count2);
        $db->commit();

        go\DB\query($patternI);
        $count3 = go\DB\query($patternS, null, 'el');
        $this->assertEquals($count2 + 1, $count3);
        $db->rollback();

        $count4 = go\DB\query($patternS, null, 'el');
        $this->assertEquals($count2, $count4);

        $db->autocommit(true);
        go\DB\query($patternI);
        $count5 = go\DB\query($patternS, null, 'el');
        $db->rollback();
    ...
    public function testPlain() {
        ...
        $count = go\DB\query($patternS, null, 'el');
        go\DB\query($patternI);
        $count1 = go\DB\query($patternS, null, 'el');
        ...
        go\DB\query($patternI);
        $count6 = go\DB\query($patternS, null, 'el');
    ...
    public function testTransactionRun() {
        ...
        $count1   = go\DB\query('SELECT COUNT(*) FROM `godb`', null, 'el');
        $callback = new goDBTestHelperTransactionRun($db, 25);

        $result = $db->transactionRun($callback->getCallback());
        $this->assertEquals(25, $result);
        $count2 = go\DB\query('SELECT COUNT(*) FROM `godb`', null, 'el');
        $this->assertEquals($count1 + 2, $count2);
    ...
    public function testTransactionLevelCommit() {
        ...
        $countB = go\DB\query('SELECT COUNT(*) FROM `godb`', null, 'el');

        $db->transactionBegin();
        $this->assertTrue($db->inTransaction());
        $this->assertEquals(1, $db->transactionLevel());
        go\DB\query('INSERT INTO `godb` SET `number`=1');

        $db->transactionBegin();
        $this->assertTrue($db->inTransaction());
        $this->assertEquals(2, $db->transactionLevel());
        go\DB\query('INSERT INTO `godb` SET `number`=2');

        $db->transactionBegin();
        $this->assertTrue($db->inTransaction());
        $this->assertEquals(3, $db->transactionLevel());
        go\DB\query('INSERT INTO `godb` SET `number`=3');
        ...
        $this->assertTrue($db->inTransaction());
        $this->assertEquals(1, $db->transactionLevel());
        go\DB\query('INSERT INTO `godb` SET `number`=4');

        $db->transactionCommit();
        $this->assertFalse($db->inTransaction());
        $this->assertEquals(0, $db->transactionLevel());

        $countE = go\DB\query('SELECT COUNT(*) FROM `godb`', null, 'el');
    ...
    public function testTransactionLevelRollback() {
        ...
        $countM = go\DB\query('SELECT COUNT(*) FROM `godb`', null, 'el');
    ...
    public function testRollbackQuiet() {
        ...
        $this->assertFalse(go\DB\query('SELECT COUNT(*) FROM `godb`', null, 'el'));
        $this->assertFalse(go\DB\query('INSERT INTO `godb` SET `number`=2'));
    ...
    public function testRollbackException() {
        ...
        $this->assertFalse($r);
        $this->assertFalse($db->inTransaction());
        $count3 = go\DB\query('SELECT COUNT(*) FROM `godb`', null, 'el');
    ...
    public function testMulti() {
        ...
        $db->transactionCommit();
        $this->assertFalse($db->inTransaction());
        $count4 = go\DB\query('SELECT COUNT(*) FROM `godb`', null, 'el');
