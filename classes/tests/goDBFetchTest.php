...
class goDBFetchTest extends goDBTestBase {
    ...
    public function testObject() {
        ...
        $result  = go\DB\query($pattern, null, 'object');
        $this->assertInternalType('array', $result);
    ...
    public function testKAssoc() {
        ...
        $result   = go\DB\query($pattern, null, 'kassoc');
        $expected = array(
        ...
        $pattern  = 'SELECT `id`,`number`,`caption` FROM `godb` ORDER BY `id` ASC LIMIT 2,3';
        $result   = go\DB\query($pattern, null, 'kassoc:number');
        $expected = array(
    ...
    public function testIteratorFree() {
        $db     = $this->db(true);
        $summ   = 0;
        $result = go\DB\query('SELECT `number` FROM `godb` LIMIT 5', null, 'icol');
    ...
    public function testVars() {
        ...
        $result  = go\DB\query($pattern, null, 'vars');
        $this->assertInternalType('array', $result);
    ...
    public function testRowAssoc() {
        ...
        $result  = go\DB\query($pattern, array(5), 'rowassoc');
        $expected = array('number' => 3, 'caption' => 'double');
        $this->assertEquals($expected, $result);
        $result  = go\DB\query($pattern, array(55), 'rowassoc');
    ...
    public function testRowRow() {
        $db      = $this->db(true);
        $pattern = 'SELECT `number`,`caption` FROM `godb` WHERE `id`=?i';
        $result  = go\DB\query($pattern, array(5), 'rowrow');
        $expected = array(3, 'double');
        $this->assertEquals($expected, $result);
        $result  = go\DB\query($pattern, array(55), 'rowrow');
        $this->assertEmpty($result);
    }
    ...
    public function testRowObject() {
        $db      = $this->db(true);
        $pattern = 'SELECT `number`,`caption` FROM `godb` WHERE `id`=?i';
        $result  = go\DB\query($pattern, array(5), 'rowobject');
        $this->assertEquals(3, $result->number);
        $result  = go\DB\query($pattern, array(55), 'rowobject');
        $this->assertEmpty($result);
    }
    ...
    public function testEl() {
        ...
        $result  = go\DB\query($pattern, array(5), 'el');
        $this->assertEquals(3, $result);
        $result  = go\DB\query($pattern, array(55), 'el');
        $this->assertEmpty($result);
    }
    ...
    public function testBool() {
        $db      = $this->db(true);
        $pattern = 'SELECT `number` FROM `godb` WHERE `id`=?i';
        $result  = go\DB\query($pattern, array(5), 'bool');
        $this->assertTrue($result);
        $result  = go\DB\query($pattern, array(55), 'bool');
        $this->assertEmpty($result);
    }
    ...
    public function testNum() {
        ...
        $result  = go\DB\query($pattern, array(5), 'num');
    ...
    public function testAr() {
        ...
        $result  = go\DB\query($pattern, null, 'ar');
        $this->assertEquals(2, $result);
