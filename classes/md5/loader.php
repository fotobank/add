              ...
              */
              ...
              private function img_watermark()
              {
                     ...
                     if (is_array($aWmImgInfo) && count($aWmImgInfo)) {
                            ...
                            imagecopy($this->image, $rWmImage, $this->imgWidth / 2 - $aWmImgInfo[0] / 2,
                                      $this->imgHeight * 0.5, 0, 0, $aWmImgInfo[0], $aWmImgInfo[1]);
                     }
              }
              ...
              private function win2uni()
              {
                     ...
                     $result = '';
                     for ($i = 0, $iMax = strlen($this->text_string); $i < $iMax; $i++) {
                            $charcode = ord($this->text_string[$i]);
                            $result   .= ($charcode > 175) ? '&#'.(1040 + ($charcode - 176)).';' :
                                   $this->text_string[$i];
                     }
                     ...
                     if ($this->hotspot != 0) {
                            ...
                            switch ($this->hotspot) {
                                   case 1:
                                          ...
                                          break;
                                   case 2:
                                          $this->txp = $the_box['left'] + ($this->imgWidth / 2) - ($the_box['width']
                                                                                                   / 2);
                                          $this->typ = $the_box['top'] + $the_box['height'] + $this->text_padding;
                                          break;
                                   case 3:
                                   ...
                                   case 4:
                                          $this->txp = $this->text_padding;
                                          $this->typ = $the_box['top'] + ($this->imgHeight / 2) - ($the_box['height']
                                                                                                   / 2);
                                          break;
                                   case 5:
                                          $this->txp = $the_box['left'] + ($this->imgWidth / 2) - ($the_box['width']
              ...
              {
                     $color_shad = imagecolorallocate($this->image, HexDec($this->rgbtsdw) & 0xff,
                                                      (HexDec($this->rgbtsdw) >> 8) & 0xff,
                                                      (HexDec($this->rgbtsdw) >> 16) & 0xff);
                     $color_text = imagecolorallocate($this->image, HexDec($this->rgbtext) & 0xff,
                                                      (HexDec($this->rgbtext) >> 8) & 0xff,
