<?php
       ...
       class debugger_errorClass
       {
              ...
              private $mailOptions = ['mail_Period' => 5, // Minimal period for sending an error message (in minutes)
                                      'from_Addr'   => "webmaster@aleks.od.ua", 'from_Name' => "Ошибки в скриптах",
              ...
              public function captureShutdown()
              {
                     ...
                     if ($error) {
                            ...
                            $aTempArr = ['TRANSLATION' => 'Остановка скрипта из-за фатальной ошибки',
                                         'SUGGESTION'  => 'Проверьте скрипт на ошибки'];
                            $errType = 'Fatal Error';
                                   ...
                                   if ($iLine === $iErrLine) {
                                          $aSourceElem[] = $this->XML_DOC->createElement('SOURCE_LINE_ERROR',
                                                 $numLine.') '.trim(iconv("WINDOWS-1251", "UTF-8",
                                                        ' /** ЛИНИЯ ОШИБКИ => */ '.htmlspecialchars($sLine))));
                                   }
                                   else {
               ...
               * показать весь текущий журнал в таблице, со статистикой (лучше всего использовать после
