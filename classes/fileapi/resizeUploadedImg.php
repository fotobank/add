 ...
 * серверный конечный скрипт обработки изображения (прием и проверка файла $_FILES и ресайц)
 */


class fileapi_resizeUploadedImg {
       ...
       private $newThumbName = ''; // имя конечного файла
       private $upload_dir = ''; // папка для загрузки
       private $maxThumbSize = 200; // max ширина конечной картинки
       ...
       private $resize = true; // false - не кадрировать


       /**
        * динамическое содание и присвоение переменых
        *
        * @param $var
        * @param $data
        */
       private function magic($var, $data) {
       ...
       public function resize() {
              ...
              if (imagecopyresampled($this->thumbImage, $sourceImage, 0, 0, 0, 0, $thumbWidth, $thumbHeight, $oryginalWidth, $oryginalHeight)
              ) {
                     $this->uploaded = true;
       ...
       public function upload() {
              ...
              if($this->resize) {
                     ...
                     if ($this->type == 'image/gif') {
                            ...
                            move_uploaded_file ($this->tmp , $this->upload_dir.$this->newThumbName);
              }
