...
function replace ($string,$id)
{
        ...
        if ($id=="answ")
		{
		$string = preg_replace ("'[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*'","<a href=\"mailto:\\0\" class=answ>\\0</a>", $string);
        $string = preg_replace ("'([[:space:]]|\n|<br>)(www.[-a-zA-Z0-9@:%_\+.~#?&//=]+)'", '\\1<a href="http://\\2" target="_blank" class=answ>\\2</a>', $string);
        $string = preg_replace ("'([[:space:]]|\n|<br>)(http://.[-a-zA-Z0-9@:%_\+.~#?&//=]+)'", '\\1<a href="\\2" target="_blank" class=answ>\\2</a>', $string);
		}
        else if ($id=="dark")
		{
		$string = preg_replace ("'[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*'","<a href=\"mailto:\\0\" class=dark>\\0</a>", $string);
		...
		{
		$string = preg_replace ("'[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*'","<a href=\"mailto:\\0\">\\0</a>", $string);
        $string = preg_replace ("'([[:space:]]|\n|<br>)(www.[-a-zA-Z0-9@:%_\+.~#?&//=]+)'", '\\1<a href="http://\\2" target="_blank">\\2</a>', $string);
        $string = preg_replace ("'([[:space:]]|\n|<br>)(http://.[-a-zA-Z0-9@:%_\+.~#?&//=]+)'", '\\1<a href="\\2" target="_blank">\\2</a>', $string);
		}
        $string = preg_replace ("'(\[img\])(http://.[-a-zA-Z0-9@:%_\+.~#?&//=]+)(\[/img\])'", '<img src="\\2">', $string);
...
function mess($name,$mess,$mail,$url,$city,$date,$answer,$messnum)
		...
		else if (($mail!="")&&($anti_email=="yes"))
		{
			...
			$mess_mail = "<a class='snd_ml' href=\"javascript:;\"  onClick=\"openBrWindow('send_mail.php?mm1=$mm1&mm2=$mm2','send_mail','scrollbars=yes,resizable=yes,width=420,height=270');return false;\"><img src=\"mail.php?mm1=$mm1&mm2=$mm2\" align=\"absmiddle\" border=0 /></a>";
		}
        echo "\n<table class='tb_mess'>\n";
        echo "<tr><td class='td_name' colspan=2>\n";
		if($city=="") { echo "<a class='sml' href=\"javascript: smile(':reply: [b]".$name."[/b] \\n');\"><b>$name</b></a></td></tr>"; }
        else { echo "<a class='sml' href=\"javaScript: smile(':reply: [b]".$name." (".$city.")[/b] \\n');\"><b>$name</b> ($city)</a></td></tr>\n"; }
...
if (isset($_POST["add"])):
	...
	if ($mail != "" && !preg_match("/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*$/i",$mail)) { $error[] = "Таких адресов e-mail не бывает."; }
	...
	if (preg_match("/[^ ]{".$maxword.",}/i",$mess)) { $error[] = "Слова в сообщении не должны превышать $maxword символов."; }
	...
	{
	  preg_match('/(\[img\])(http://.[-a-zA-Z0-9@:%_\+.~#?&//=]+)(\[/img\])/i',$temp_string,$pic_array);
		$imgurl = $pic_array[2];
...
///////////   загружаем  форму   ////////////
if (!preg_match("/\%/i", $TABWIDTH)) $SENDWIDTH = $TABWIDTH-300;
else $SENDWIDTH = 300;
include("$send_form");
...
echo "<table><tr><td class='td_klmess'><b>Всего сообщений : $lines</b></td></tr><tr><td class='td_klmess'><b>Страницы : |$print_pages</b></td></tr></table>";
echo "<table border=0 cellpadding=0 cellspacing=0 width=100% height=4><tr><td height=4></td></tr></table>";
...
echo "<table><tr><td class='td_klmess'><b>Страницы : |$print_pages</b></td></tr><tr><td class='td_klmess'><b>Всего сообщений : $lines</b></td></tr></table>";
