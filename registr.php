...
<center><div><h2><b><hremind>Регистрация на сайте:</hremind></b></h2></div></centerp><br>

	<div id="for_reg_cont">
	Регистрация необходима для хранения, покупки или бесплатного скачивания фотографий из фотобанка.
Для всех зарегистрированных пользователей, активно принимающих участие в голосованиях за фотографии, предусмотрены скидочные бонусы и акции,
а для пользователей, чьи фотографии набрали пять и более звездочек рейтинга в альбоме - бесплатная печать 
на профессиональном оборудовании.
<br>
Пожалуйста, внимательно заполните все поля и нажмите кнопку "отправить".
Указывайте реально существующий email, на него будут приходить ссылки для 
скачивания выбранных Вами фотографий.
Внимание! В целях безопасности, никому не передавайте свои логин и пароль!
Пароль и логин могут состоять только из ЛАТИНСКИХ букв, цифр или подчеркивания. 
Желательно использовать пароль длиной больше семи символов, включающий в себя цифры,
а также большие и маленькие буквы. 
	</div>
    <br>
    <div id="form_reg">
...
//   $mdPassword = md5($rPass);
   ...
   $cnt = intval(mysql_result(mysql_query('select count(*) cnt from users where login = \''.mysql_escape_string($rLogin).'\''),0));
   if($cnt > 0)
     die('Пользовател с таким логином уже существует!');
   $cnt = intval(mysql_result(mysql_query('select count(*) cnt from users where email = \''.mysql_escape_string($rEmail).'\''),0));
   if($cnt > 0)
...
?>
    ...
    <form action="" method="post" enctype="multipart/form-data" style="display:<? echo "$frm_reg" ?>">
    <table>
    <tr>
    <td> Логин: </td> <td><input class="inp_f_reg" name="rLogin"></td>
    </tr>
    <tr>
     <td> Пароль: </td> <td> <input class="inp_f_reg" type="password" name="rPass"></td>
     </tr>
     <tr>
     <td>Пароль ещё раз:</td> <td><input class="inp_f_reg" type="password" name="rPass2"></td>
     </tr>
      <tr>
     <td>Ваше имя:</td> <td><input name="rName_us" class="inp_f_reg" type="text"></td>
     </tr>
     <tr>
     <td>E-mail:</td> <td><input name="rEmail" class="inp_f_reg" type="text"></td>
     </tr>
    </table>
    <br>
