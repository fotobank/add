...
//									print "checked link: {$_SERVER['REQUEST_URI']}<br />\n";
			    		...
			    		<div style="font-size: 24px;">Изменения записаны!</div></div>';
									...
									if ($result) {
												unset($result['terms']);
												if ($result['pass'] === '') {
                               unset($result['pass'], $result['pass2']);
                        } else {
															$result['pass'] = md5($result['pass']);
															unset($result['pass2']);
												}
												$result['mail_me'] = $result['mail_me'] ?? 'off';
						...
						//			print "<br>link invalid: {$_SERVER['REQUEST_URI']} \n";
									err_exit ('Прямой вход на данную страницу запрещен. Для входа используте кнопку "пользователь1"', '/error.php');
						}
