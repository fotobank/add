<?php
       ...
       if (isset($_POST['createpdf'])) {
              ...
              if (extension_loaded('zip')) { // Проверка расширения ZIP
                     if (isset($post['files']) and count($post['files']) > 0) { // Проверка выбранных файлов
                            $zip      = new ZipArchive(); // Загрузить zip-библиотеку
                            $zip_name = time().".zip"; // Zip имя
                            if ($zip->open($zip_name, ZIPARCHIVE::CREATE) !== true) { // Открытие zip-файла для загрузки файлов
                                   $error .= '* Ошибка при создании ZIP-архива<br/>';
                            }
