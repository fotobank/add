	...
	if (extension_loaded('zip')) { // Checking ZIP extension is available
		if (isset($post['files']) and count($post['files']) > 0) { // Checking files are selected
			...
			foreach ($post['files'] as $file) {
				require_once(__DIR__.'./../../core/users/downloadZip.php');
...
if ($orders) {
	...
	foreach ($orders as $order) {
		...
		<div class="order-tabl">
			...
			<form name="zips" method="post" action="<?= $link ?>">
				<table class="table table-condensed table-hover table-striped">
					<thead>
					<tr>
						<th>#</th>
						<th><input type="checkbox" id="<?= $order['id'] ?>"/></th>
						<th>Имя файла</th>
						<th>Дата заказа</th>
					...
					<tbody>
					<?
					$orderFoto = $db->query('SELECT *
									FROM download_photo
									WHERE id_order = ?i', array($order['id']), 'assoc');
					if ($orderFoto) {
						foreach ($orderFoto as $key => $foto) {
							...
							<tr class="<?= $trClass ?>">
								<td><?=$key + 1?></td>
								<td align="center">
									<input type="checkbox" name="files[]" class="<?= $order['id'] ?>" value="<?= $foto['download_key'] ?>"/>
								</td>
					...
					<tr class="info">
						<td colspan="5" style=" text-align: center;">
							<input type="submit" name="createpdf" class="btn btn-primary" value="Заархивировать и скачать">
						</td>
					</tr>
					</tbody>
				</table>
...
<table width="600" border="1" align="center" cellpadding="10" cellspacing="0" style="border-collapse:collapse; border:#ccc 1px solid;">
	...
	<tr>
		<td colspan="3" align="center">
			<input type="submit" name="createpdf" class="btn btn-success" value="Заархивировать и скачать">&nbsp;
			<input type="reset" name="reset" class="btn btn-success" value="Сброс">
		</td>
	</tr>
</table>