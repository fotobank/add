  ...
  <ol class="commentlist">
	 ...
	 function commentParents ($post, &$komments) {
		...
		$rez .=  "<div id= 'div-comment-{$post['id']}' class='comment-body'>
				...
				<p>{$post['text']}</p>
				<div class='reply'>
				  <a onclick=\"return addComment.moveForm('div-comment-{$post['id']}', '{$post['parents_id']}', 'respond', '{$post['id']}', '{$post['news_id']}');\"
				  href=\"index.php?replytocom=".$post['id']."#respond\">Ответить</a>

				  ".if_adm("<a onclick=\" $('#redaktor').toggle(); $('#respond').toggle(); $('#cancel-comment-reply-link').toggle();
						ajaxPostNews('/canon68452/ajax/ajaxPostNews.php', '', 'idKomments={$post['id']}&idNews={$post['news_id']}');
  ...
  <div class="block">
	 ...
	 <div class="cancel-comment-reply">
		<small><a rel="nofollow" id="cancel-comment-reply-link"  style="display:none;"
			href="#respond">Нажмите, чтобы отменить ответ.</a></small>
	 </div>
	 <form action="/canon68452/index.php?page=8" method="post" id="commentform">
		<p><input type="text" name="author" id="author" value="<?=if_adm('Jurii')?>" size="22" tabindex="1" class="textarea"/>
		  <label for="author"><small>Имя (обязательно)</small></label></p>
		<p><input type="text" name="email" id="email" value="<?=if_adm('aleksjurii@gmail.com')?>" size="22" tabindex="2" class="textarea"/>
		  <label for="email"><small>E-mail (не публикуется) (обязательно)</small></label></p>
		<p><input type="text" name="url" id="url" value="<?=if_adm('http://www.aleks.od.ua')?>" size="22" tabindex="3" class="textarea"/>
  ...
  <div id="redaktor" style="display:none;">
	 <div class="block">
		...
		<form action="/canon68452/index.php?page=8" method="post" id="commentform">
		  ...
		  <p><label for="comment_r"></label><textarea name="comment_r" id="comment_r" cols="100%" rows="10" tabindex="4"></textarea></p>
		  <p>
