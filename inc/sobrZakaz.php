 ...
 */
  ...
  require_once (__DIR__.'/lib/zn_ftp/zn_ftp.php');
...
else
{
  ...
  if($data['id_dost'] == 'Самовывоз из студии (в Одессе)')
	 {
		$param_index = $db->query('SELECT `param_index` FROM `nastr` WHERE `param_value` = ?string',array($data['adr_studii']),'el');
  ...
  $name_dir1 = '№'.$data['id'].' Получ. '.$data['name'].' '.$data['subname'].' Всего-'.$data['format'].' '.$koll.'шт. Долг-'.$dolg;
...
if($FTP_HOST_FROM && $FTP_USER_FROM && $FTP_PSWD_FROM)
  {
	 ...
	 $ftp->set_path($zakaz);
	 if($ftp->is_dir($name_dir1)==false)  $ftp->mkdir($name_dir1);
	 $ftp->set_path($zakaz.$name_dir1.'/');
	 if($ftp->is_dir($name_dir2)==false)  $ftp->mkdir($name_dir2);
	 foreach($photo_data as $foto)
		{
		  $from = $db->query('select p.ftp_path
															 ...
															 AND o.id_photo = p.id
															 AND o.id_photo = ?i',
			 array($foto['id_print'],$foto['id_photo']), 'el');
		  $ftp->set_path($to);
		  $foldKoll = 'По '.$foto['koll'].'шт';
		  if($ftp->is_dir($foldKoll)==false)  $ftp->mkdir($foldKoll); // создаем папку кол-во фотографий
		  $file = $FTP_HOST_FROM.$from;  // Абсолютный путь до скачиваемого файла начиная с www директории (доступной из веба)
