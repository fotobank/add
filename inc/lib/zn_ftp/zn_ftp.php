  ...
  public function cp($source, $dest)
	...
	public function upload($file, $ftp_file, $check_form_upload=false)
	{
	  ...
	  if ($test != "f")
		...
		$ftp_file_ar = explode("/", $ftp_file); // разбить на массив
		$ftp_file_name = array_pop($ftp_file_ar); // отрезать от массива последний элемент - имя файла получателя
		if (count($ftp_file_ar) != 1)
		{
			$ftp_file_up = implode("/", $ftp_file_ar); // собрать массив в строку
		}
		else
		{
			$ftp_file_up = "/";
	...
	private function _check_path($path)
	{
	  ...
	  /* Бинарная строка, либо символы не в windows-1251,UTF-8 */
	  $result = mb_detect_encoding($path, "windows-1251,UTF-8");
	  if ($result === false)
		 {
			throw new Exception("Путь задан неверно. Бинарная строка, либо символы не в windows-1251 или UTF-8", 183);
		 }
