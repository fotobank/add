	...
	*/
	...
	$.Montage.prototype 		= {
		...
		_cropImage			: function( $img, w, h, cropParam ) {
			...
			// if image is already wrapped, unwrap it
			if( $img.parent('a.am-wrapper').length > 0 ) {
				$img.unwrap();
				dec=0;
			}
			...
			$wrapper	= $('<a />').addClass('am-wrapper').css({
       ...
       //    $img.removeAttr('rel').removeAttr('title');
		...
		_reload				: function() {
			...
			if( new_el_w !== this.cache.container_w ) {
				...
				var instance 				= this;
				instance.$imgs.removeClass('montage').unwrap().each( function(i) {
		...
		_create 			: function( options ) {
			...
			instance.$imgs		= instance.element.children('img');
		...
		add					: function( $images, callback ) {
			...
			this.$imgs 		= this.$imgs.add( $images );
