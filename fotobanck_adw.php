...
?>
							...
							<div id="static" class="modal hide fade in animated fadeInDown" data-keyboard="false" data-backdrop="static" tabindex="-1" aria-hidden="false" >
										 ...
										 <div class="modal-body" >
														...
														<form action="/fotobanck_adw.php" method="post" >
																	 <label for="inputError" class="ttext_red" style="float: left; margin-right: 10px;" >Пароль: </label > <input id="inputError"
																					type="text" name="album_pass" value="" maxlength="20" /> <input class="btn-small btn-primary" type="submit" value="ввод" />
														</form >
										 </div >
							...
							if ($ostPop <= 0 || $ostPop == 5) {
										 ...
										 <div id="zapret" class="modal hide fade" tabindex="-1" data-replace="true" style=" margin-top: -180px;" >
														<div class="err_msg" >
																	 ...
																	 <div class="modal-body" >
																					<div style="color:black" >Вы использовали 5 попыток ввода пароля.В целях защиты, Ваш IP заблокирован на 30 минут.
																					</div >
																					<br >
																					<?
																					...
																					<br > <br > <a href="/kontakty.php" ><span class="ttext_blue" >Восстановление пароля</span ></a > <a style="float:right"
																								 class="btn btn-danger" data-dismiss="fotobanck_adw.php" href="/fotobanck_adw.php?back_to_albums" >Закрыть</a >
																	 </div >
														</div >
										 </div >
							<?
							...
							if ($may_view) {
										 ...
										 if ($rs) {
														...
														foreach ($rs as $key => $ln) {
																	 ...
																	 if ((($kollFoto == $koll))) {
																					?>
																					<a class="modern" style="position: absolute; float: right;" href="/dir.php?num=<?= substr(($ln['img']), 2, -4) ?>"
																								 title="Фото № <?= $ln['nm'] ?>" > <img id="<?= substr(trim($ln['img']), 2, -4) ?>" class="lazy" <?=$sz_string?>
																												src="" data-original="/thumb.php?num=<?= substr(trim($ln['img']), 2, -4) ?>"
																												alt="№ <?= $ln['nm'] ?>" />№ <?=$ln['nm']?>
																					</a >
																					</div>
																					<?
																	 ...
																	 <?
																					...
																					<a class="modern" style="position: relative; float: left; margin-right: <?= $margin; ?>px;"
																								 href="/dir.php?num=<?= substr(($ln['img']), 2, -4) ?>" title="Фото № <?= $ln['nm'] ?>" > <img
																												id="<?= substr(trim($ln['img']), 2, -4) ?>" class="lazy" <?=$sz_string?> src=""
			 ...
			 function verifyParol($may_view) {
							...
							if (!$may_view && $current_album != NULL) {
										 ...
										 <div class="row" >
														<div class="page" >
																	 <a class="next" href="/fotobanck_adw.php?back_to_albums" >« назад</a > <a class="next" href="/fotobanck_adw.php" >« попробовать
																																																																										 еще раз</a >
														</div >
			 ...
			 function top5($may_view, &$rs, &$ln, &$source, &$sz, &$sz_string) {
						   ...
						   order by votes desc, id asc limit 0, 5', array($session->get('current_album')), 'assoc');
										 ...
										 if ($rs) {
                								 ...
                								 </span ><div id="d<?= substr(trim($ln['img']), 2,	-4) ?>" style="width: 146px;" >
														...
														foreach ($rs as $ln) {
																	 ...
																	 <div id="foto_top" >
																					<a class="modern" href="/dir.php?num=<?= substr(($ln['img']), 2, -4) ?>" title="Фото № <?= $ln['nm'] ?>"
																								 data-placement="bottom" data-original-title="Фото № <?= $ln['nm'] ?>" >
																								 <figure class="ramka" >
																												<span class="top_pos" style="opacity: 0;" ><?=$pos_num?></span > <img class="lazy"
																															 data-original="thumb.php?num=<?= substr(trim($ln['img']), 2, -4) ?>"
																															 id="<?= substr(trim($ln['img']), 2, -4) ?>" src="" alt="<?= $ln['nm'] ?>"
			 ...
			 if ($event == 'on') {
                  		...
                  		".$collapse_nm."
										...
										<div class="page" >
													...
													<a class="next" href="/fotobanck_adw.php?back_to_albums" >« раздел "<?=$razdel?>"</a > <a class="next" >« альбом "<?=$album_data['nm']?>
																																																																	"</a >
										</div >
