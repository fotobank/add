...
if(mysql_num_rows($rs) > 0)
{
	...
	<tbody>
  	...
  	while($ln = mysql_fetch_assoc($rs))
  	{
		...
		<? if ($ln['status'] == 1) {$podtw = 'да';} else {$podtw = 'нет';} ?>
...
<td style="text-align: center">
	...
	?>
	<div class="btn-group">	
    <button class="btn btn-success dropdown-toggle" data-toggle="dropdown"><?=$foto_zak.' шт'?><span class="caret"></span></button>    
	<?
    ...
    if(mysql_num_rows($rez_fakt) > 0)	
      ...
      ?>
		...
		<img href="<?=$ln_foto['foto_folder'].$ln_foto['id_album'].'/'.$ln_foto['img']?>" src="<?=$ln_foto['foto_folder'].$ln_foto['id_album'].'/'.$ln_foto['img']?>" alt="<?=$ln_foto['pnm']?>" 
		title="<?=$ln_foto['pnm']?>" <?=$sz_string?> />       
		<h6 style="margin-top: 0px; margin-bottom: 0px;">Фото № <?=$ln_foto['pnm']?></h6>
    ...
    <form action="index.php?pg=<?=$pg?>" method="post" style="margin-bottom: 0px;">
