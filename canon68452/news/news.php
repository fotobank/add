<?
 if (!isset($_SESSION['admin_logged'])) die();
  ...
  if(isset($_GET['nid'])) {
	 ...
	 if(go\DB\query('SELECT COUNT(*) FROM `news` WHERE `id`=?i',array($_GET['nid']),'el')==0)
		{
		  echo '<div class="title2">Извините ,данная новость не существует
	           <br/><a href="index.php">К новостям</a></div>';exit;
		}
	 go\DB\query('UPDATE `news` SET `pros`=`pros`+1 WHERE id=?i',array($_GET['nid']));

	 $news = go\DB\query('SELECT * FROM `news` WHERE id=?i',array($_GET['nid']),'row');
...
<a href="newsadm.php?act=newsedit&amp;id='.$_GET['nid'].'">[edit]</a>'; echo if_adm($str);
	 ...
	 if(go\DB\query('SELECT COUNT(*) FROM `config` WHERE common=?i',array(1),'el')==1 && $news['komm']==1){
		echo '<a href="news/komm.php?nid='.$_GET['nid'].'">Прокомментировать</a> ['.go\DB\query('SELECT COUNT(*) FROM `komments` WHERE `news_id`=?i',array($_GET['nid']),'el').']<br/>';}
