...
$mp = mysql_fetch_array(mysql_query('SELECT * FROM `config` WHERE id=1'));
  $db = go\DB\Storage::getInstance()->get('db-for-data');
  $mp = $db->query('SELECT * FROM `config` WHERE id=1','','row');
...
switch(@$_GET['act']){
	...
	admnick="'.mysql_real_escape_string($_POST['login']).'" &&	admpass="'.md5($_POST['pass']).'"'),0)==1){
		...
		<div class="content2"><a href="?act=index">Админка</a></div>';
			...
			<div class="content2"><a href="?">Авторизация</a></div>';include 'sys/end.php';exit;}
			break;
