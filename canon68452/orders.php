				...
				<div id="<?= $n ?>" class="tab-pane <?= $akt ?>">
					<table class="table table-striped table-bordered table-condensed span8">
						...
						foreach ($tmp as $tmp2) {
							...
							<tr>
								<td colspan="1">
									<img alt="<?= $tmp2['pnm'] ?>" src="<?=$tmp2['foto_folder'].$tmp2['id_album'].'/'
									 .$tmp2['img'] ?>" alt="<?= $tmp2['pnm'] ?>" title="<?= $tmp2['pnm'] ?>" <?=$sz_string?> />
								</td>
								<td align="center"> фотография № <a href="<?=
									$tmp2['foto_folder'].$tmp2['id_album'].'/'.$tmp2['img'] ?>"><?=$tmp2['pnm']?></a></td>
