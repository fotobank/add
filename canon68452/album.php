                ...
                if ($ftp_host && $ftp_user && $ftp_pass)
                    {
                        ...
                        if ($file_list === false)
                            {
                                ftp_close($ftp);
                                $out = "<div class='alert alert-error'>Директория не существует! <br></div>";
                                senderror($out, $id, '');
...
//echo 'Ответ ftp: <br><pre>', print_r($file_list,1), '</pre>';
                        ...
                        if (!count($file_list))
                            ...
                            {
                                $out = "<div class='alert alert-error'>Список доступных файлов пустой!<br>Или заданная на FTP папка : $album_data[ftp_folder] - пустая!</div>";
                                senderror($out, $id, '');
...
//var_dump($ftp,$f_name);
                                        if (!ftp_get($ftp, $local_file, $remote_file_old, FTP_BINARY))
                                            {
                                                ftp_close($ftp);
                                                $out = "<div class='alert alert-error'>Не могу загрузить файл: $remote_file -> $local_file  </div> ";
                                                senderror($out, $id, '');
                                ...
                                else
                                    {
                                        ...
                                        $out = "<div class='alert alert-error'>$remote_file - это папка или неподдерживаемый файл!<br></div>";
                                        senderror($out, $id, '');
                                        @unlink($local_file);
                                    }
//   }
                        ...
                        ftp_close($ftp);
                        $out = "<div class='alert alert-info'>Загруженно $key фотографий.</div>";
                        sendtext($out, $id, '');
                        $out = '';
                        sendtext($out, $id, 'bar');
                    }
...
<div class="row">
    <div class="span5 offset2">
        ...
        <div>4.Перед изменением <b>папки в фотобанке</b> для закаченного альбома необходиммо сначала создать папку на
            сервере!
        </div>
    </div>
</div><?
                ...
                <form id="myForm1" action="index.php" method="post">
                    <select id="appendedInputButton" class="span3" name="id" style="height: 28px;" onClick="$('#myForm1').trigger('submit');">
                        ...
                        while ($ln_cat = mysql_fetch_assoc($rs_cat))
                            {
                                ?>
                                <option value="<?= $ln_cat['id'] ?>" <?=($current_c == $ln_cat['id'] ? 'selected="selected"' : '')?>> <?=$ln_cat['nm']?></option>
                            <?
                            }
                        ?>
