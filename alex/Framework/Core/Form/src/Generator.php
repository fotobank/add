<?php
       ...
       class Generator
       {
              ...
              public function __construct()
              {
                     ...
                     //config defaults
                     $this->config['validator']['value']         = 'Validator.php';
                     $this->config['validatorClass']['value']    = 'Framework\Core\Form\Validator';
               ...
               *                * validator <str> имя файла класса валидатора (Validator.php)
               *                * validatorClass <str> имя класса валидатора (валидатор)
               *                * sanitize <bool> дезинфицировать ввод (true)
               *                * submitMessage <str> сообщение, отображаемое при успешной отправке (форма успешно
               *        отправлена!)
               *                * submitField <str> идентификатор кнопки отправки
               *                * showDebug <bool> показывать сообщения об исключениях
               *                * linebreaks <str>, что использовать es linebreaks
               *                * divs <bool> использовать divs для инкапсуляции строк ярлыков и полей
               *                * showErrors <bool> показать сообщения об ошибках проверки
               *                * errorTitle <str> название списка ошибок
               *                * errorLabel <str> добавляет это к метке недопустимого поля
               *                * errorPosition <enum> указать положение окна с ошибкой [до | после | in_before |
               *        in_after]
               *                * («before» и «after» помещает ящик вне элемента формы, параметры in_ * помещают его
               *        внутрь)
               *                * showAfterSuccess <bool> показать форму после успешной отправки (true)
               *                * cleanAfterSuccess <bool> очистить поля после успешной отправки (true)
               */
               ...
               *
               * Для радиокнопок, списка и наборов флажков ассоциативный массив
               * должен быть задан как параметр $aux(name => value).
               * Этот массив будет использоваться для создания списка.
               */
              /**
               * @param      $type
               * @param      $id
                     ...
                     try {
                            if ($this->form_validator === null) {
                                   ...
                                   if (class_exists($this->getConfig('validatorClass'))) {
                                          ...
                                          throw new InvalidArgumentException('Validator класс &quot;'
                            ...
                            if ($function !== null && method_exists($this->form_validator, $function)) {
                                   ...
                                   throw new InvalidArgumentException('Метод &quot;'.$function.'&quot; не найден.');
                            }
                     }
                     catch (LogicException $e) {
                            if($this->debug()){
              ...
              * @param $label label of item
               ...
               *
               * Этот метод пытается предотвратить отправку форм ботами.
               * Маленькая строка js заполняет скрытое поле с заданным кодом проверки,
               * и класс проверяет этот код на отправке. Это предотвращает форму
               * предоставление ботами, не поддерживающими JS, но также может предотвратить
               * юридические представления, когда пользователь отключил JS.
               */
              /**
               * @param        $code
               * @param string $field
               ...
               *
               * Метод проверки должен вернуться ложным, если поле действует, иначе он должен вернуть сообщение об
               *        ошибке
               */
